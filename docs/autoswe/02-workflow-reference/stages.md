...
This document defines the required reasoning, minimal artifact, and gate for each design stage.

The model must perform the reasoning.
The model does not need to emit all of that reasoning.
...
## Global Compression Rule

Each stage must be completed.

Each stage output should contain only the information needed to constrain downstream work.

Do not expand a stage artifact into full prose unless compact output would lose necessary constraints.
